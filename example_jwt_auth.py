...
def main():
    ...
    print("=== JWT Authentication Examples ===")

    # Method 1: Using the high-level main utils function
    print("\n1. Using get_jwt_authenticated_client_or_error():")
    try:
        client = get_jwt_authenticated_client_or_error(jwt_token)
        print("✓ Successfully created authenticated client using main utils")

        # Test the client by fetching licenses
        licenses_response = get_data(client, "/v2/licenses")
        print(f"✓ Found {len(licenses_response.get('data', []))} licenses")

    except Exception as e:
        print(f"✗ Error: {e}")

    # Method 2: Using the lower-level auth utils function
    print("\n2. Using create_jwt_authenticated_client():")
    try:
        client = create_jwt_authenticated_client(jwt_token)
        print("✓ Successfully created authenticated client using auth utils")
    ...
    print("\n3. Using JWT authentication with custom configuration:")
    try:
        ...
        print("✓ Successfully created client with custom config")
